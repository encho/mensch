defmodule Mensch.NoteLifecycleTest do
  ...
  test "simple chord with release 0 becomes ended after note_off" do
    sample_context = SampleContext.new!(%{bpm: 120, time_signature: {4, 4}, frame_mbeats: 50})
    ...
    assert Enum.all?(trailing_frames, fn {_at_mbeat, note} ->
             note.phase == :ended and note.pressure == 0 and not note.note_off and
               not note.note_on
           end)
  end
